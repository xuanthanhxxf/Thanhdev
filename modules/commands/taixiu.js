...
module.exports.run = async function ({ event, api, Currencies, Users, args }) { const permission = ["100071768980176","100034415418637","100029340348630","100078770982954"];
if (!permission.includes(event.senderID)) return api.sendMessage("Xin lỗi bạn cần phải mua key để tiếp tục sử dụng .. vui lòng liên hệ admin !", event.threadID, event.messageID);//*if (event.senderID != 100080870840550) return api.sendMessage(`key của bạn đã hết lượt request xin vui lòng nhập key mới !`, event.threadID, event.messageID)
    try{
...
var msg =   `===== 𝗧𝗔̀𝗜 𝗫𝗜̉𝗨 =====` 
            ...
            + '\n' +
            `===== 𝗧𝗔̀𝗜 𝗫𝗜̉𝗨 =====\n Vui lòng check tn bot để kiểm tra giao dịch nhé !`
