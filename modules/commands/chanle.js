	...
	try {
		...
		switch (r[0]) {
			...
			case "-c": {
				...
				if (parseInt(r[1]) < 500000) return e.sendMessage("Số tiền phải lớn hơn hoặc bằng 500000", s, o);
				const g = await a.getData(n.senderID);
				if (g.money < parseInt(r[1])) return e.sendMessage(`Bạn không có đủ ${r[1]} để tạo bàn game mới vui lòng dùng lệnh work hoặc ăn xin admin!!`, s, o);
